function [STATM] = plot_fields_biogem_3d_k_Dom_euxinia(PEXP1,PEXP2,PVAR1,PVAR2,PT1,PT2,PIK,PMASK,PCSCALE,PCMIN,PCMAX,PCN,PDATA,POPT,PNAME)
...
%     end
    ...
    %
    if ~isempty(overlaydataid)
        % DOM: set shape depending on euxinia
        if (data_shapecol == 'n'),
            for n = 1:nmax,
...
%                 overlaydata_shape(n) = 'o';
    ...
    %
    % *** CREATE COLOR BAR ************************************************** %
    % Dom: no color bar: comment below out
...
if (isempty(overlaydataid) && ~isempty(exp_2) && (plot_main == 'n') && (plot_secondary == 'n'))
    ...
    % create netCDF file
    ncid = netcdf.create([filename, '.', str_date, '.nc'],'NC_WRITE');
