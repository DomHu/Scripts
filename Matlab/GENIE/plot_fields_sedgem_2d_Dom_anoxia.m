...
%Fanny - contour O2
anox = 5;
[C,h] = contour(xm_ex,sin(pi*ym_ex/180.0),zm_ex,[anox anox],'g-');
set(h,'LineWidth',1.5);
...
% Dom: from euxinia fct
    for n = 1:nmax,
            if (data_siteonly == 'n'),
                scatter(overlaydata(n,1),overlaydata(n,2),4,overlaydata(n,4),overlaydata_shape(n),'k','Filled','LineWidth',data_sitelineth,'Sizedata',data_size,'MarkerEdgeColor',overlaydata_ecol(n));
            else
...
%     if (data_siteonly == 'n')
%         scatter(overlaydata(:,1),overlaydata(:,2),4,overlaydata(:,3)/data_scale,overlaydata_shape(n),'k','Filled','LineWidth',data_sitelineth,'Sizedata',data_size,'MarkerEdgeColor',overlaydata_ecol(n));
%     else
%         if (overlaydata_fcol(n) == '-'),
