function [Total_Ox_capac, OUTPUT] = plot_fields_biogem_3d_k_ox_capac(PEXP1,PEXP2,PVAR1,PVAR2,PT1,PT2,PIK,PMASK,PCSCALE,PCMIN,PCMAX,PCN,PDATA,POPT,PNAME)
% plot_fields_biogem_3d_k_ox_capac
%
...
eval(POPT);
%DH: don't plot figure
plot_main = 'n';        % [  '']  PLOT THE MAIN FIGURE
% set date
...
if (data_anomoly == 'y')
    ...
    elseif (plot_minval == 'y')
        ...
        data = data_1 - 2*data_2;   %DH: O2 - 2xH2S
%       was: data = data_1 - data_2;
    end
else
    data = data_1;
end
            ...
            if topo(j,i) == 0.0
                ...
                for k = data_kmin:data_kmax
                    if ((data(k,j,i) > -1.0E6) && (data(k,j,i) < 0.9E36))
                      % zm(j,i) = zm(j,i) + data_M(k,j,i)*data(k,j,i)*393444720640.0;  % DH: multiple with area of grid cell to get total inventory not per normalized area
...
overlaydata_zm(:,:) = zm(:,:);
% DH: save vector of combined oxidizing capacity in .mat file
Vec_Ox_cap = reshape(zm,1,[]);
% DH
if(false)
    filename; %= [PNAME, '.mat'];
    filename2 = [exp_1(1:8), '_', dataid_1, '_', 'k', num2str(kplot),'_Ox_capac']
    save([filename2, '.mat'],'Vec_Ox_cap')
    % calculate mean concetration and return to calling fct
    % Mean_H2S = nanmean(Vec_Ox_cap)
end
Total_Ox_capac = nansum(Vec_Ox_cap)
%
% *********************************************************************** %
...
% DH        plot_crossplotc(loc_x_data,loc_y_data,[],loc_x_label,loc_y_label,'',POPT,[par_pathout '/' filename '.CROSSPLOT']);
% DH       plot_crossplotc(loc_x_data,loc_y_data,loc_D_data,loc_x_label,loc_y_label,loc_D_label,POPT,[par_pathout '/' filename '.CROSSPLOTD']);
        %
    end
    %
...
% DH    plot_histc_2d(data_vector_1,loc_bins1,strrep(dataid_1,'_','-'),[],[],'',plot_histc_SETTINGS,str_name);
    % double histogram
...
% DH        plot_histc_2d(data_vector_1,loc_bins1,strrep(dataid_1,'_','-'),data_vector_2,loc_bins2,[strrep(dataid_2,'_','-')],plot_histc_SETTINGS,[str_name 'D']);
    else
        loc_bins2 = fliplr(grid_zt_edges');
        loc_bins2(find(loc_bins2 < plot_D_min)) = [];
        loc_bins2(find(loc_bins2 > plot_D_max)) = [];
% DH        plot_histc_2d(data_vector_1,loc_bins1,strrep(dataid_1,'_','-'),data_vector_D,loc_bins2,'Depth (m)',plot_histc_SETTINGS,str_name);
    end
    %
    % ******************************************************************* %
    %
