function [OUTPUT] = plot_fields_biogem_3d_k_depthmax(PEXP1,PEXP2,PVAR1,PVAR2,PT1,PT2,PIK,PMASK,PCSCALE,PCMIN,PCMAX,PCN,PDATA,POPT,PNAME,PDEPTHMAX, PLOT_HIST)
% plot_fields_biogem_3d_k_depthmax
% DH: To plot the depth where the maximum concentration occurs in the water-column
% set PDEPTHMAX = true/false
%
%   *******************************************************************   %
...
if(plot_depth_max_conc)
    contour_plot = 'n';     % [ 'y']  OVERLAY CONTOUR PLOT?
    zm_maxvalue = zeros(jmax,imax);
    zm_maxvalue(:,:) = zm(:,:);
    zm(:,:) = zm_maxk(:,:);
end
overlaydata_zm(:,:) = zm(:,:);
...
% DH        fid = fopen([par_pathout '/' filename '_MODELPOINTS', '.', str_date '.dat'], 'wt');
        ...
        for j = 1:jmax,
            if topo(j,i) > layb(j,i)
                ...
                if (isnan(zm(j,i)))
                    ...
                    if(plot_depth_max_conc)
                    if(zm_maxvalue(j,i)<10e-6)    % DH: plot in white if Max(H2S)<10mumol/kg
                        h = patch([lonw(j,i) lonw(j,i) lone(j,i) lone(j,i)],[lats(j,i) latn(j,i) latn(j,i) lats(j,i)],[1 1 1]);
                        set(h,'EdgeColor',[1 1 1]);
                    else
...
%     end
    ...
    if (plot_format_old == 'y')
        if (plot_depth_max_conc)
            if (par_mutlab > 2015),
                print('-dpsc2', '-bestfit', [par_pathout '/' filename '.' str_date '_DepthMax_Conc.ps']);
            else
                print('-dpsc2', [par_pathout '/' filename '.' str_date '_DepthMax_Conc.ps']);
            end
        else
             if (par_mutlab > 2015),
    ...
    % *** SAVE DATA ***************************************************** %
    %
% DH    fprint_1Dn_d([data_vector_1 data_vector_D],[par_pathout '/' filename '.kDATA.', str_date, '.res']);
    %
    % ******************************************************************* %
    %
end
...
% *********************************************************************** %
%
if(plot_depth_max_conc || ~PLOT_HIST) % DH: don't plot histograms for depth of maximum concentration
    plot_secondary = 'n';
end
if (plot_secondary == 'y'),
    %
