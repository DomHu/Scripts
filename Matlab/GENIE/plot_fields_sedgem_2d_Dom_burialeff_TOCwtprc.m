...
% *********************************************************************** %
%
% Dom: save matrix with burial efficiencies!
% Burial_efficiencies_ksulf_7 = zm;
% save('Burial_efficiencies_ksulf_E+7.mat','Burial_efficiencies_ksulf_7')
...
data_size = 50;
if ~isempty(overlaydataid)
        % DOM: set shape depending on BW anoxia
    if (data_shapecol == 'n'),
        for n = 1:nmax,
%            overlaydata_shape(n) = 'o';
            switch overlaydata(n,4)
            case 1  % BW anoxia YES
                overlaydata_shape(n) = 'o';
            case 0  % BW anoxia NO
                overlaydata_shape(n) = 'x';
            otherwise % 2 BW YES with ?
                overlaydata_shape(n) = '^';
            end
    ...
    % plot overlay data
% Dom: from euxinia fct
    for n = 1:nmax,
            if (data_siteonly == 'n'),
% with colors                scatter(overlaydata(n,1),overlaydata(n,2),4,overlaydata(n,3)/data_scale,overlaydata_shape(n),'Filled','LineWidth',data_sitelineth,'Sizedata',data_size,'MarkerEdgeColor',overlaydata_ecol(n));
                scatter(overlaydata(n,1),overlaydata(n,2),4,'k',overlaydata_shape(n),'Filled','LineWidth',data_sitelineth,'Sizedata',data_size,'MarkerEdgeColor',overlaydata_ecol(n));
            else
...
if (plot_secondary == 'n'),
    ...
    if ( ~isempty(dataid_2) || (~isempty(overlaydataid) && (data_only == 'n')) ),
        ...
        if ~isempty(dataid_2),
            ...
            % Dom: reshap ocean depth for colours in crossplot
            loc_grid_topo = reshape(grid_topo(:,:),1,[]);
            loc_grid_topo(loc_grid_topo>0.0)=NaN;
            %
        ...
        %
%   no color     plot_crossplotc(loc_x_data,loc_y_data,[],loc_x_label,loc_y_label,'',POPT,[filename '.CROSSPLOT']);
        % DOM: give ocean depth to plot colors
        % observationson x-axis:
        % plot all data
%        plot_crossplotc(loc_y_data, loc_x_data,-loc_grid_topo,loc_y_label,loc_x_label,'',POPT,[filename '.CROSSPLOT'], 'n');
        % no 0 wt% data
        % Dom create my title:
